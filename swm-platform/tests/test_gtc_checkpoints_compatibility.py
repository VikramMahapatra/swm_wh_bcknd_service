...
def test_gtc_checkpoints_list():
    resp = client.get("/gtc-checkpoints", headers={"x-role": "admin"})
...
def test_gtc_checkpoints_list_filters(params):
    resp = client.get("/gtc-checkpoints", params=params, headers={"x-role": "admin"})
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)

def test_gtc_checkpoints_create():
    payload = {
        "truck_id": "TST-TRUCK",
        "arrived_at": "2026-05-23T10:00:00Z",
        "is_dry": True,
        "is_wet": False,
        "is_metal": False,
        "is_plastic": False,
        "is_sanitary": False,
        "truck_cleanliness_score": 5.0,
        "gtc_cleanliness_score": 4.5,
        "remarks": "Test checkpoint"
    }
    resp = client.post("/gtc-checkpoints", json=payload, headers={"x-role": "admin"})
