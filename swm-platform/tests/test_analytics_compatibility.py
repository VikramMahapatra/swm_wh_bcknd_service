...
@pytest.mark.parametrize("endpoint,expected_type,expected_keys", [
    ("/analytics/performance/overview", dict, ["efficiency", "total_trips", "total_distance_km", "avg_idle_pct"]),
    ("/analytics/performance/zone-wise", list, ["zone", "efficiency", "total_trucks", "total_trips"]),
    ("/analytics/performance/vendor-wise", list, ["vendor", "efficiency", "total_trucks", "total_trips"]),
    ("/analytics/predictions/maintenance", list, []),
    ("/analytics/trends/collection-rate", list, ["date", "collection_rate", "trips_count"]),
])
def test_analytics_endpoints(endpoint, expected_type, expected_keys):
    ...
    if expected_type is dict:
        for key in expected_keys:
            assert key in data
    elif expected_type is list and expected_keys:
        if data:
            for key in expected_keys:
